...
test.describe('State on DOM element (o.state pattern)', () => {
  ...
  test('counters render and have state on el._bw_state', async ({ page }) => {
    ...
    // Should find 3 counter cards (bw_is_component inside the flex wrapper)
    const counters = page.locator('#counter-demo > div > .bw_is_component');
    ...
    const values = page.locator('#counter-demo > div > .bw_is_component h4 + div');
    ...
    const stateOnElement = await page.evaluate(() => {
      var els = document.querySelectorAll('#counter-demo > div > .bw_is_component');
      var results = [];
  ...
  test('counters are independent — clicking one does not affect others', async ({ page }) => {
    ...
    const stateAfter = await page.evaluate(() => {
      var results = [];
      document.querySelectorAll('#counter-demo > div > .bw_is_component').forEach(function(el) {
        if (el._bw_state && typeof el._bw_state.count === 'number') {
          results.push(el._bw_state.count);
        }
      });
  ...
  test('todo list works with state on element', async ({ page }) => {
    ...
    const todoState = await page.evaluate(() => {
      var els = document.querySelectorAll('#todo-demo .bw_lc');
      var result = null;
      els.forEach(function(el) {
  ...
  test('state survives re-render (bw.DOM preserves _bw_state)', async ({ page }) => {
    ...
    const btn = page.locator('#counter-demo > div > .bw_is_component').nth(0).locator('button', { hasText: '+' });
    for (let i = 0; i < 5; i++) {
      await btn.click();
  ...
  test('theme switcher works with state on element', async ({ page }) => {
    ...
    const themeState = await page.evaluate(() => {
      var result = null;
      document.querySelectorAll('#css-demo .bw_lc').forEach(function(el) {
