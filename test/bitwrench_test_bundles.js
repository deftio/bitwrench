...
const CORE_FUNCTIONS = [
  ...
  'escapeHTML', 'hexToHsl', 'makeStyles', 'css', 'loremIpsum',
  'hslToHex', 'adjustLightness', 'mixColor', 'relativeLuminance',
  'textOnColor', 'deriveShades', 'derivePalette',
  'colorParse', 'colorRgbToHsl', 'colorHslToRgb', 'colorInterp',
...
const UTIL_COLOR_BUNDLES = {
  ...
  es5Min:  'dist/bitwrench-util-color.es5.min.js',
};
...
describe('Dist Bundle Smoke Tests', function () {
  ...
  // -------------------------------------------------------------------------
  // 8. bitwrench-util-color bundles
  // -------------------------------------------------------------------------

  describe('bitwrench-util-color bundles', function () {

    describe('ESM (bitwrench-util-color.esm.js)', function () {
      let mod;
      before(async function () {
        mod = await loadESM(UTIL_COLOR_BUNDLES.esm);
      });

      it('exports expected named exports', function () {
        for (const name of UTIL_COLOR_EXPORTS) {
          assert.ok(name in mod, `ESM should export "${name}"`);
        }
      });
    });

    describe('ESM min (bitwrench-util-color.esm.min.js)', function () {
      let mod;
      before(async function () {
        mod = await loadESM(UTIL_COLOR_BUNDLES.esmMin);
      });
    ...
    for (const [label, path] of [
      ['CJS', UTIL_COLOR_BUNDLES.cjs],
      ['CJS min', UTIL_COLOR_BUNDLES.cjsMin],
      ['UMD', UTIL_COLOR_BUNDLES.umd],
      ['UMD min', UTIL_COLOR_BUNDLES.umdMin],
      ['ES5', UTIL_COLOR_BUNDLES.es5],
      ['ES5 min', UTIL_COLOR_BUNDLES.es5Min],
    ]) {
      describe(`${label} (${path})`, function () {
        let info;
        before(function () {
