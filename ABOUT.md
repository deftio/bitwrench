...
`bw.component()` provides reactive state management. `bw.css()` generates CSS from JS objects. `bw.generateTheme()` creates complete design systems from two hex colors. `bwserve` streams TACO objects over SSE for server-driven UIs. The whole thing ships as a single 39KB gzipped file with no dependencies.
