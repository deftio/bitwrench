...
TACO options (`o.state`, `o.render`, `o.handle`, `o.slots`) provide explicit state management. `bw.css()` generates CSS from JS objects. `bw.makeStyles()` creates complete design systems from two hex colors. `bwserve` streams TACO objects over SSE for server-driven UIs. The whole thing ships as a single 40KB gzipped file with no dependencies.
