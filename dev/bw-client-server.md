...
bw.apply(msg)                // apply a received protocol message to DOM
bw.parseJSONFlex(str)        // parse relaxed-format string -> JS object

// Connection object (managed by bwclient.js, auto-generated by shell)
...
Design rationale:
- `bw.parseJSONFlex()` instead of `bw.parseJSON()` — avoids confusion with real JSON since it accepts non-standard relaxed format (single quotes, trailing commas)
...
| `batch`   | `operations[]`               | sequential `bw.apply()` per operation                |
...
3. Device serves static shell page from flash memory (PROGMEM)
   ...
   - Sets up polling with bw.apply()/bw.parseJSONFlex() to '/bw/ui' with {
       transport: 'poll',
...
4. Polling loop starts:
   ...
   c. bw.parseJSONFlex() converts single quotes -> strict JSON -> JS object
   d. bw.apply() dispatches: type:'replace' -> bw.DOM('#content', node)
...
5. User clicks a button (data-bw-action="toggle-led")
   ...
   e. bw.apply() processes the response
...
`bw.parseJSONFlex()` converts relaxed format to strict JSON via a character-by-character state machine. Handles:
...
<body>
  ...
  <script>
    ...
    // relaxedJson:true tells bw.parseJSONFlex() to accept single-quoted JSON
    // from the firmware, which is much easier to build in C/C++.
    // Polling loop using bw.apply() + bw.parseJSONFlex()
    // (connection management now handled by bwclient.js for SSE;
    //  embedded polling uses manual setInterval + fetch)
    var conn = { /* see bwclient.js for SSE connection API */
      transport: 'poll',
      interval: 1000,
      target: '#content',
      relaxedJson: true
...
void handleUI() {
  ...
  // bw.parseJSONFlex() on the browser side converts this to strict JSON.
...
3. Server starts on port 3000, generates shell page:
   ...
   - bwclient.js opens SSE to /bw/events, sends actions to /bw/return/action

4. Browser navigates to localhost:3000
   a. GET / -> server returns shell page
   b. Browser loads bitwrench and bwclient.js
   c. bwclient.js opens EventSource to /bw/events (SSE)
   ...
   g. Browser receives SSE event -> JSON.parse -> bw.apply() -> bw.DOM()
   h. Page renders
...
5. Every 2 seconds, server's onInterval fires:
   ...
   c. Each browser: bw.apply() -> bw.patch('clock', '3:42:15 PM')
...
6. User clicks button (data-bw-action="refresh"):
   ...
   e. Browser receives SSE event -> bw.apply() -> DOM updated
...
```javascript
// Shell page or bw.apply handler for 'module' message type:
...
All client-side protocol dispatch (`bw.apply`) resolves targets via CSS selectors. For uuid-addressed elements (the common case in server-driven UI), bitwrench maintains a live hash map for O(1) lookup instead of DOM traversal.
...
All server-to-client messages (SSE, poll, module) converge on `bw.apply()`.
...
1. **Opt-in on the client.** `exec` messages are rejected unless the
   ...
   ```javascript
   // allowExec: true must be set in the server options or shell configuration
   ```
