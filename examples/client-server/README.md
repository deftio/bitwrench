...
- **Server-push rendering**: `client.mount()` sends TACO over SSE, browser renders via `bw.apply()`
- **Targeted patches**: `client.patch(ref, { text })` updates individual elements without full re-render
...
Browser                    Server (Node.js)
  ...
  |                           |
  |  GET /bw/events/:id       |
  |  <-- SSE: replace #app    |   (client.mount() sends TACO)
