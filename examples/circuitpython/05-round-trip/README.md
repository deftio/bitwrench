...
A settings form posts values back to the device, which stores them and re-serves the UI.

Deliberately clunky: every save refetches the whole page. Example 6 fixes that.

**Try it:** change the name and press Save, then reload -- the device kept it.

**Routes:** `GET /`, `GET /ui.json`, `POST /set`, `GET /js/bitwrench.js`
