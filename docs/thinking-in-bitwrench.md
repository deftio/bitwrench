...
Change `cardStyle` once, both cards update on the next render. No preprocessor needed.
...
Change `brand` once, every rule that references it updates. No build step.
...
**Path L -- local.** The handler runs in the browser. This is what `onclick: function() { ... }` does. The function closes over local variables and can call `bw.mount()`, `bw.refresh()`, or any other API.
...
### Important: never attach handlers in o.mounted

When a component re-renders (after `bw.refresh()`), old DOM children are replaced. Listeners attached via `addEventListener` in `o.mounted` are silently lost. Use `onclick` in `a:` -- it is re-attached automatically on every render:
...
Use `mounted` for setup that needs the real DOM element -- observers, measuring dimensions, initializing third-party libraries. Use `unmount` to clean up. Never use `mounted` for event handlers (use `a: { onclick: fn }` instead).
...
bw.parseRJSON("{ name: 'Alice' }"); // relaxed JSON (unquoted keys, single quotes)
```
